...
def DashboardCard(
    symbol: str,
    icon: Union[solara.Element, str] = None,
    market_cap: Optional[str] = None,
    market_cap_change_percentage: Optional[str] = None,
    pending: Optional[bool] = False,
):
    ticker_data, set_ticker_data = solara.use_state(cast(Optional[TickerData], None))
    ...
    result: solara.Result[bool] = solara.use_thread(fetch_data, intrusive_cancel=False)
    if result.error:
        raise result.error
    ...
    if not ticker_data:
            ...
            with solara.Div():
                with solara.Div(
                    ...
                    with solara.GridFixed(
                        ...
                        solara.Text(
                            "loading", style={"font-size": "1.5rem", "font-weight": 500}
                        )
        ...
        with rv.Card(
            ...
            rv.CardTitle(
                children=[icon], style_="padding: 0px 0px; padding-bottom: 5px;"
            )
            with solara.Div(
                style={
                    "display": "inline",
                ...
                with solara.GridFixed(
                    ...
                    if market_cap is not None:
                        solara.Text(str("market cap"), style={"font-size": "0.6rem"})
                    solara.Text(
                        f"{ticker_data.price_change_percent}%",
                        style={
                            "color": processColor(ticker_data.price_change_percent),
                            "font-weight": 500,
                        },
                    )
                    solara.Text(str("24h change price"), style={"font-size": "0.6rem"})
                    if market_cap_change_percentage is not None:
                        solara.Text(
                            str(market_cap_change_percentage) + "%",
                            style={
                                "color": processColor(ticker_data.price_change_percent),
                                "font-weight": 500,
                            },
                    ...
                    if market_cap_change_percentage is None:
                        solara.Text("Height", style={"font-size": "0.6rem"})
                    if market_cap_change_percentage is None:
                        solara.Text(
                            str(f"{ticker_data.low_price}$"),
                            style={"font-size": "0.6rem"},
                        )
                    if market_cap_change_percentage is None:
                        solara.Text("Low", style={"font-size": "0.6rem"})
...
def get_available_symbols():
    ...
    try:
        ...
        available_symbols = sorted(
            list(
                set([symbol["baseAsset"].lower() for symbol in symbols_data["symbols"]])
            )
        )
...
def Page():
    ...
    available_symbols, _ = solara.use_state(
        solara.use_memo(get_available_symbols, dependencies=[])
    )
    ...
    with solara.VBox() as main:
        ...
        coingecko_data, set_coingecko_data = solara.use_state(
        ...
        result: solara.Result[bool] = solara.use_thread(
            fetch_data, intrusive_cancel=False
        )
        if result.error:
            raise result.error
        ...
        if "status" in coingecko_data:
            ...
            for i, symbol in enumerate(init_app_state.value):
                ...
                if coingecko_data_for_symbol:
                    ...
                    card = DashboardCard(binance_symbol, binance_symbol).key(symbol)
                    dashboard_cards.append(card)

        solara.use_memo(
            lambda: set_grid_layout(
                [
                    {
                        "h": 6,
                        "i": str(i),
                        "moved": False,
                        "w": row_widths[i % len(row_widths)],
                        "x": sum(row_widths[: i % len(row_widths)]),
                        "y": (i // len(row_widths)) * 6,
                    }
